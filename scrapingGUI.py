...
class Ui_MainWindow(QWidget):
    ...
    def showTableWeb(self) :
        ...
        if "" not in keywords:
            ...
            if len(dhave) > 0:
                ...
                # print(type(self.wt.result),self.wt.result)
                # print(len(self.dt))
                # self.wt.any_signal.connect(self.upgradeProgressWeb)
    ...
    def deleteButton_3(self) : #สำหรับปุ่ม delete tab web
        ...
        #print(deleteKey)
        self.dt = dm.deletekeyword(list(deleteKey))
        return
    ...
    def exportWeb(self) :
        ...
        sd = self.dateSinceReturnWeb()
        ed = self.dateUntilReturnWeb()
        fname = '_'.join(keywords)+'_'+str(sd)+'_'+str(ed)+'.csv'
        if not os.path.exists(os.path.join("Web Export",fname)):
            path = os.path.join("Web Export",fname)
        else:
            n=1
            newname = os.path.join("Web Export",'_'.join(keywords)+'_'+str(sd)+'_'+str(ed)+"("+str(n)+")"+'.csv')
            path = os.path.join("Web Export",newname)
            while os.path.exists(path):
                n+=1
                newname = os.path.join("Web Export",'_'.join(keywords)+'_'+str(sd)+'_'+str(ed)+"("+str(n)+")"+'.csv')
                path = os.path.join("Web Export",newname)

        # if type(self.dt) != None :
